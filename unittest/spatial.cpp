...
BOOST_AUTO_TEST_CASE(test_Inertia)
{
  ...
  // Test Inertia parametrizations
  {
    ...
    std::cout << "Pseudo-inertia matrix from log Cholesky:\n" << pseudo << std::endl;
    ...
    std::cout << "Inertia from log cholesky:\n" << I_from_log_cholesky << std::endl;
    ...
    std::cout << "Pseudo-inertia matrix from Inertia:\n" << pseudo_from_inertia << std::endl;
    ...
    std::cout << "Jacobian of log Cholesky parametrization:\n" << jacobian << std::endl;

    // Check if determinant is non-zero
    BOOST_CHECK(std::abs(jacobian.determinant()) > 1e-10);

    // Check if log-cholesky parametrization to pseudo-inertia gives same result as "on-site"
    // calculations
    ...
    std::cout << "Pseudo-inertia matrix from on-site calculations:\n" << pseudo_chol << std::endl;
    ...
    std::cout << "Inertia from pseudo inertia:\n" << I_from_log_cholesky << std::endl;

    // Convert Inertia to dynamic parameters
    Eigen::Matrix<double, 10, 1> dynamic_params_inertia = I_from_log_cholesky.toDynamicParameters();
    std::cout << "Dynamic parameters from Inertia:\n"
              << dynamic_params_inertia.transpose() << std::endl;

    // Convert log Cholesky to dynamic parameters
    Eigen::Matrix<double, 10, 1> dynamic_params_log_cholesky = log_cholesky.toDynamicParameters();
    std::cout << "Dynamic parameters from log Cholesky:\n"
              << dynamic_params_log_cholesky.transpose() << std::endl;
    ...
    Eigen::Matrix<double, 10, 1> dynamic_params_pseudo_inertia =
      pseudo_inertia.toDynamicParameters();
    std::cout << "Dynamic parameters from Pseudo Inertia:\n"
              << dynamic_params_pseudo_inertia.transpose() << std::endl;
